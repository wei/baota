        ...
        for filespath in files:
            ...
            if os.path.getsize(os.path.join(root,filespath))<1024000:
                ...
                for rule in rulelist:
                    ...
                    if result:
                        tmp = {}
                        tmp['filename'] = os.path.join(root,filespath);
                        tmp['code'] = str(result[0][0:200])
                        tmp['etime'] = time.strftime('%Y-%m-%d %H:%M:%S',time.localtime(os.path.getmtime(os.path.join(root,filespath))))
                        data.append(tmp);
                        break
    print('           可疑文件         ')
    print('*'*30)
