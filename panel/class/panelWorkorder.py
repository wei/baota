...
import datetime
import selectors
import ssl, socket, sys, os, time, json, re, gzip, pwd
...
bp_workorder = Blueprint("workorder", __name__, url_prefix="/workorder")
bp_workorder_socket = Blueprint("workorder_socket", __name__,
                                url_prefix="/workorder")
...
def get_headers():
    headers = {
        "User-Agent": "BT PANEL WORKORDER CLIENT/VERSION 1.0"
    }
    return headers


@bp_workorder.route("/get_user_info")
def user_info():
    try:
        ...
        pass
    return public.returnMsg(False, user_tip)


@bp_workorder.route("/close", methods=["post"])
def workorder_close():
    try:
        from flask import jsonify
        import requests
        data = request.values or request.json or request.args
        ...
        if user_info:
            ...
            response = requests.post(server, headers=get_headers(), data=data)
...
@bp_workorder.route("/create", methods=["post"])
def workorder_create():
    try:
        ...
        collect = request.form.get("collect", False)
        other = None
        if collect:
            other = get_error_log()
...
def workorder_list():
    ...
    try:
        ...
        response = requests.get(server, headers=get_headers(), data=user_info)
        if response.status_code == 200:
            return jsonify(response.json())
    except Exception as e:
        pass
...
@bp_workorder.route("/get_messages")
def get_messages():
    data = request.args
    workorder = data.get("workorder")
    ...
    response = requests.get(server_url, headers=get_headers(), data=data)
    if response.status_code == 200:
...
def start_workorder_client(clients, workorder, ping_interval,
                           on_message,
                           on_error,
                           on_close,
                           on_pong,
    ...
    try:
        ...
        print(e)
        # public.WriteLog("DEBUG", str(e))


def check_client_status(workorder):
    """检查客户端连接状态"""
...
@bp_workorder_socket.route("/client")
def workorder_client(ws):
    """面板端客户端通信连接"""

    data = request.args or request.json or request.values
    ...
    if workorder not in panel_clients.keys():
        panel_clients[workorder] = {}

    if workorder not in ping_record.keys():
        ping_record[workorder] = datetime.datetime.now()
    ...
    def _on_message(socket_bp_workorder, message):
        ...
        try:
            ...
            if temp_message:
                # 转发
                for key, _ws in panel_clients.get(workorder, {}).items():
    ...
    def _on_close(sock):
        ...
        try:
            panel_clients[workorder].pop(session_id)
        except Exception as e:
            if debug:
    ...
    try:
        ...
        start_workorder_client(workorder_clients, workorder,
                               ping_interval, _on_message, _on_error, _on_close,
                               _on_pong, _on_open)
        ...
        def read_websocket_data(socket, mask):
            ...
            while True:
                ...
                send = False
                retry = 0
                while not send and retry < max_retry:
                    try:
                        if check_client_status(workorder):
                            sock = workorder_clients.get(workorder)
                            # 限制ping间隔时间
                            if message == "ping":
                                last_ping_time = ping_record.get(workorder,
                                                                 None)
                                ...
                                interval = (
                                                   now - last_ping_time).seconds - ping_interval
                                if not last_ping_time or interval >= 0 or abs(
                                        interval) <= 2:
                                    sock.sock.ping()
                                    ping_record[workorder] = now
                                    if debug:
                                        print("ping")
                                # else:
                                #     ws.send("pong")
                            ...
                            if debug:
                                print("检查客户端：")
                                print(panel_clients.get(workorder))
                            for key, _ws in panel_clients.get(workorder,
                                                              {}).items():
                                if key != session_id:
                                    if debug:
                    ...
                    if connections[workorder]:
                        ...
                        start_workorder_client(workorder_clients,
                                               workorder, ping_interval,
        ...
        if session_id not in panel_clients[workorder].keys():
            panel_clients[workorder][session_id] = ws
        # public.WriteLog("All Session ID:", str(panel_clients[workorder].keys()))

        while True:
            if not connections[workorder] or retry >= max_retry:
                break
            if debug:
                print("selecting...")
            try:
                if ws.handler.socket.fileno() < 0:
                    break
                events = epoll.select(ping_interval)
        ...
        try:
            ...
            if workorder in panel_clients.keys():
                for key, _ws in panel_clients[workorder].items():
                    if _ws and not _ws.closed:
