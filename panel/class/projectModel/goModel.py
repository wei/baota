...
class main(projectBase):
    ...
    _go_logs_path = '{}/vhost/logs'.format(_go_path)
...
echo $! > {pid_file}'''.format(
    ...
    def project_add_domain(self,get):
        ...
        if success_list:
            ...
            return public.return_data(False, "成功添加{}个域名，失败{}个!".format(len(success_list), len(error_list)),
                                      error_msg="成功添加{}个域名，失败{}个!".format(len(success_list), len(error_list)))
