...
class main:
    ...
    def edit_project_remark(self,args):
        ...
        dp.write_log("项目 [{}] 的备注修改成功 [{}] --> [{}]!".format(stacks_info['name'],stacks_info['remark'],public.xsssec(args.remark)))
    ...
    def edit_template_remark(self,args):
        ...
        dp.write_log("修改模板 [{}] 备注成功 [{}] --> [{}]!".format(stacks_info['name'],stacks_info['remark'],public.xsssec(args.remark)))
        dp.sql("templates").where("id=?", (args.templates_id,)).update(pdata)

    def create_project_in_path(self,name,path):
        shell = "cd {} && /usr/bin/docker-compose -p {} up -d &> {}".format("/".join(path.split("/")[:-1]), name,self.__log_file)
    ...
    def remove(self,args):
        ...
        shell = "/usr/bin/docker-compose -p {} -f {} down &> {}".format(public.md5(statcks_info['name']), statcks_info['path'],self.__log_file)
        a,e=public.ExecShell(shell)
    ...
    def stop(self,args):
        ...
        shell = "/usr/bin/docker-compose -p {} -f {} stop &> {}".format(public.md5(statcks_info['name']), statcks_info['path'],self.__log_file)
        a,e=public.ExecShell(shell)
    ...
    def start(self,args):
        ...
        shell = "/usr/bin/docker-compose -p {} -f {} start > {}".format(public.md5(statcks_info['name']), statcks_info['path'],self.__log_file)
        a, e = public.ExecShell(shell)
    ...
    def restart(self,args):
        ...
        if not statcks_info:
            return public.returnMsg(True, "没找到项目配置!")
        shell = "/usr/bin/docker-compose -p {} -f {} restart &> {}".format(public.md5(statcks_info['name']), statcks_info['path'],self.__log_file)
        a, e = public.ExecShell(shell)
    ...
    def pull(self,args):
        ...
        shell = "/usr/bin/docker-compose -p {} -f {} pull &> {}".format(statcks_info['name'], statcks_info['path'],self.__log_file)
        a, e = public.ExecShell(shell)
    ...
    def pause(self,args):
        ...
        if not statcks_info:
            return public.returnMsg(True, "没找到项目配置!")
        shell = "/usr/bin/docker-compose -p {} -f {} pause &> {}".format(public.md5(statcks_info['name']), statcks_info['path'],self.__log_file)
        a, e = public.ExecShell(shell)
    ...
    def unpause(self,args):
        ...
        if not statcks_info:
            return public.returnMsg(True, "没找到项目配置!")
        shell = "/usr/bin/docker-compose -p {} -f {} unpause &> {}".format(public.md5(statcks_info['name']), statcks_info['path'],self.__log_file)
        a, e = public.ExecShell(shell)
    ...
    def add_template_in_path(self,args):
        ...
        if not create_failed and create_successfully:
            ...
            return {'status': True,'msg': '添加模板失败: 模板名已经存在或格式错误 [{}]'.format(','.join(create_failed))}
        return {'status': True, 'msg': '这些模板成功: [{}]<br>这些模板失败: 模板名已经存在或格式错误 [{}]'.format(','.join(create_successfully), ','.join(create_failed))}
