...
class panelSearch:
    ...
    def __init__(self):
        ...
        if not public.M('sqlite_master').where(
                'type=? AND name=?', ('table', 'panel_search_log')).count():
    ...
    def insert_settings(self, rtext, exts, path, mode, isword, iscase, noword,
                        backup_path):
        inser_time = self.dtchg(int(time.time()))
        data = {
            "rtext": rtext,
            "exts": json.dumps(exts),
            "path": path,
            "mode": mode,
            "isword": isword,
            "iscase": iscase,
    ...
    def get_dir(self,
        ...
        if is_backup:
            ...
            zfile = zipfile.ZipFile(back_zip,
                                    "w",
        ...
        for i in return_data:
            for i2 in exts:
                ...
                if i3[-1] == i2:
                    ...
                    temp = self.get_files_lin(i, text, mode, isword, iscase,
                                              noword, is_backup, rtext, zfile)
                    if temp:
                        if rtext or noword:
        ...
        if is_backup:
            if zfile:
                zfile.close()
            self.insert_settings(rtext, exts, path, mode, isword, iscase,
                                 noword, back_zip)
            return True
        return result
    ...
    def get_exts_files(self,
        ...
        if is_subdir == 0:
            return self.get_dir_files(path, exts, text, mode, isword, iscase,
                                      noword, is_backup, rtext)
        elif is_subdir == 1:
            return self.get_dir(path, exts, text, mode, isword, iscase, noword,
                                is_backup, rtext)
    ...
    def get_search(self, args):
        ...
        exts = exts.split(',')
        is_tmpe_files = self.get_exts_files(path, exts, text, mode, isword,
                                            iscase, noword, is_subdir)
