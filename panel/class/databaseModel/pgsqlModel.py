...
class main(databaseBase,panelPgsql):
    ...
    def AddDatabase(self,args):
        ...
        @添加数据库
        """
        if not args.get('name/str',0):return public.returnMsg(False, '数据库名不能为空！')
        import re
    ...
    def check_cloud_database_status(self,conn_config):
        ...
        try:
            ...
            data = sql_obj.query("SELECT datname FROM pg_database;")
            if type(data) == str:
                return public.returnMsg(False,'连接远程pgsql失败，请按以下方式排查：<br/>1、数据库端口是否正确且防火墙放行<br/>2、数据库帐号密码是否正确<br/>3、pg_hba.conf是否添加客户端放行记录<br/>4、postgresql.conf文件将listen_addresses配置加入正确的服务器IP.')
