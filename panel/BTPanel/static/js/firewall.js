var firewall = {
	...
	safety: {
		...
		removePortRule: function (row) {
			...
			layer.confirm('是否删除当前端口规则,是否继续？', { btn: ['确认', '取消'], icon: 3, closeBtn: 2, title: '删除规则' }, function (index) {
				layer.close(index);
				bt_tools.send(
					{ url: '/safe/firewall/remove_rules', data: { data: JSON.stringify(row) } },
					function (rdata) {
						bt.msg(rdata);
						if (rdata.status) {
							that.portRuleTable();
		...
		ipRuleTable: function () {
			...
			return bt_tools.table({
				...
				column: [
					...
					{
						fid: 'area',
						title: 'IP归属地&nbsp;' + (parseInt(bt.get_cookie('ltd_end')) < 0 ? '<a href="javascript:;" class="btlink" onclick="bt.soft.updata_ltd(true)">企业版专享</a>' : ''),
						template: function (row) {
							var area = row.area;
						...
						[
							{
								...
								type: 'textarea',
								readonly: isEdit,
								style: 'height: 80px;width: 200px;line-height: 22px;' + (isEdit ? 'background-color: rgb(238, 238, 238);' : ''),
								placeholder: '请输入IP地址',
							},
							{
								label: '策略',
							...
							!isEdit
								? {
										type: 'tips',
										list: ['支持添加IP：如果添加多个IP请用","隔开', '支持添加IP段,如：192.168.0.0/24', '支持添加IP范围,格式如：192.168.1.xx-192.168.1.xx，暂不支持跨网段范围'],
		...
		removeIpRule: function (row) {
			...
			layer.confirm('是否删除当前IP规则,是否继续？', { btn: ['确认', '取消'], icon: 3, closeBtn: 2, title: '删除规则' }, function (index) {
				layer.close(index);
				bt_tools.send(
					{ url: '/safe/firewall/remove_ip_rules', data: { data: JSON.stringify(row) } },
					function (rdata) {
						bt.msg(rdata);
						if (rdata.status) {
							that.ipRuleTable();
	...
	ssh: {
		...
		setTemplateSshkey: function () {
			...
			layer.open({
				...
				btn: ['提交', '关闭'],
				content:
					'<div class="bt-form bt-form pd20">' +
								...
								<div class="form-item">\
									...
									<div class="form-content ' +
										...
										<input type="checkbox" ' +
											...
											' id="' +
											item.name +
											'_alarm" class="btswitch btswitch-ios" ' +
											checked +
											' name="' +
											item.name +
											'" />\
										<label class="btswitch-btn mr5" for="' +
											item.name +
											'_alarm"></label>' +
									...
									$('.news-channel .help-info-text li')
										.eq(0)
										.text(tits.join('/') + '只能同时开启一个');
									$('.news-channel .bt-form-new')
										.unbind('click')
										.on('click', '.installNotice', function (ev) {
		...
		setSshKeyView: function () {
            ...
            <textarea id="ssh_text_key" class="bt-input-text mb10" style="height:220px;width:360px;line-height: 22px;">' +
							rdata.msg +
							'</textarea>\
		...
		getSshInfo: function () {
			bt_tools.send(
				...
				function (rdata) {
					...
					$('#fail2ban').html(
						rdata.fail2ban ? '<a href="javascript:;" class="btlink" data-type="open">已安装，查看详情</a>' : '<a href="javascript:;" style="color: red;" data-type="install">未安装，点击安装</a>'
					);
					$('#sshDetailed').html(
						'<a href="javascript:;" class="btlink" data-index="1">成功：' +
							error.success +
							'</a><span style="margin: 0 8px">/</span><a href="javascript:;" style="color: red;" data-index="2">失败：' +
							error.error +
							'</a>'
					);
		...
		loginLogsTable: function (param) {
			...
			var logs = [
				['All', '日志', 'get_ssh_list'],
				['Success', '成功日志', 'get_ssh_success'],
				['Error', '失败日志', 'get_ssh_error'],
			];
			var type = logs[param.type][0],
				tips = logs[param.type][1];
			var that = this;
	...
	safeDetect: {
		...
		safeDetectList: function () {
            ...
            <div class="progress_item_body"></div>\
				...
				function safeCount(score) {
					return (
						'<span>' +
						score +
						'</span> <span>分</span><svg viewBox="0 0 100 100"><path d="\
          M 50 50\
          m 0 -46\
          a 46 46 0 1 1 0 92\
          a 46 46 0 1 1 0 -92\
          ...
          a 46 46 0 1 1 0 -92\
          " stroke="' +
						(score === 100 ? '#20a53a' : '#fc6d26') +
						'" fill="none" stroke-linecap="round" stroke-width="5" class="el-progress-circle__path" style="stroke-dasharray: ' +
						(289.027 * score) / 100 +
						'px, 289.027px; stroke-dashoffset: 0px; transition: stroke-dasharray 0.6s ease 0s, stroke 0.6s ease 0s;"></path></svg>'
					);
				}
				var html =
				...
				function setProInfo(config) {
					...
					if (config.progEnd) {
						...
						progCircleBar
							.eq(1)
							.addClass('security')
							.css('color', 100 - config.total_score === 100 ? '#20a53a' : '#fc6d26')
							.html(safeCount(100 - config.total_score))
							.removeClass('hide')
							.prevAll()
							.addClass('hide');
	...
	renderLogsPages: function (pages, p, num) {
		return (
			(num >= pages ? '<a class="nextPage" data-page="1">首页</a>' : '') +
