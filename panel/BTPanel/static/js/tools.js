	...
	table: function (config) {
		...
		ReaderTable.prototype = {
			...
			$reader_content: function (data, page) {
				...
				do {
					...
					for (var j = 0; j < column.length; j++) {
						...
						if (i === 0 && !this.init) {
							...
							thead +=
								'<th><span ' +
								(item.sort ? 'class="not-select ' + sortName + (item.sortValue ? ' sort-active' : '') + ' cursor-pointer"' : '') +
								' data-index="' +
								j +
								'" ' +
								...
								'>' +
								(item.type == 'checkbox'
									? '<label><i class="cust—checkbox cursor-pointer ' + checkboxName + '" data-checkbox="all"></i><input type="checkbox" class="cust—checkbox-input"></label>'
									: '<span>' + item.title + '</span>') +
								(item.sort ? '<span class="glyphicon glyphicon-triangle-' + (sortValue == 'desc' ? 'bottom' : 'top') + ' ml5"></span>' : '') +
								'</span></th>';
							if (i === 0) {
						...
						if (rows !== undefined) {
							var template = '',
								className = 'event_' + item.fid + '_' + this.random;
							if (item.template) {
				...
				if (!this.init) {
					$(this.config.el + ' .divtable').append(
	...
	line: function (config) {
		var $line = $(
			'<div class="line" style="' +
				...
				'">' +
				((typeof config.must !== 'undefined' && config.must != '' ? '<span class="color-red mr5">' + config.must + '</span>' : '') + config.label || '') +
	...
	form: function (config) {
		...
		ReaderForm.prototype = {
					...
					return (
						...
						(typeof data.label !== 'undefined'
							? '<span class="tname" ' +
							  ...
							  '>' +
							  (typeof data.must !== 'undefined' && data.must != '' ? '<span class="color-red mr5">' + data.must + '</span>' : '') +
							  data.label +
							  '</span>'
							: '') +
						'<div class="' +
						...
						'"' +
						_that.$verify(that.$reader_style($.extend(data.style, labelWidth ? { 'margin-left': labelWidth } : {})), 'style') +
						'>' +
			...
			$reader_form_find: function (item) {
				var that = this,
					html = '',
					style = that.$reader_style(item.style) + _that.$verify(item.width, 'width', 'style'),
						...
						switch (item.type) {
							...
							default:
								html +=
									...
									(item.icon ? 'id="' + _event + '"' : '') +
									' class="bt-input-' +
									(item.type !== 'select_path' && item.type !== 'number' && item.type !== 'password' ? item.type : 'text') +
									' mr10 ' +
			...
			$reader_select: function (item, style, attribute, index) {
				...
				return (
					...
					'">' +
					(title || item.placeholder) +
					'</span><span class="glyphicon glyphicon-triangle-bottom ml5"></span></span>' +
			...
			$reader_secondaryMenu: function (item, style, attribute, index) {
				...
				$.each(item.list, function (key, items) {
					list +=
						'<li class="item-parent"><div class="item-menu-title">' +
						items.title +
						'</div>' +
						(function () {
							var _con = '';
							if (items.child.length > 0) {
								_con =
									'<span class="glyphicon glyphicon-triangle-right ml5"></span><div class="item-menu-body-list">\
								<ul class="">' +
									(function () {
										var str = '';
										$.each(items.child, function (key, child) {
											if (child.value == item.value || child.value == that.data[item.name]) {
												active = child;
											}
											str +=
												'<li class="item-child' +
									...
									switch (items.type) {
										...
										case 'custom_select_item':
											...
											if (config.type == 'multipleSelect') {
												...
												$(this).parent().prev().find('.glyphicon-triangle-bottom').before(_html);
	...
	msg: function (param1, param2) {
		...
		if (typeof param1 === 'object') {
			if (typeof param1.status === 'boolean') {
				...
				if (!param1.status) config = $.extend(config, { time: !param2 ? 0 : 3000, closeBtn: 2, shade: 0.3, shadeClose: true });
			}
		}
