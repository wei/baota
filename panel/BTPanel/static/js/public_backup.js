var bt = {
			...
			success: function () {
				...
				$('#bt_select').on('click', function () {
					...
					trList.each(function(){
						...
						_type = $data.type;
						path.push(bt.rtrim($data.path, '/').replace('//','/'))
					});
					if(path.length === 0) {
	 ...
	 */
	simple_confirm: function (config, callback) {
		layer.open({
			type: 1,
			title: config.title,
			area: '430px',
...
bt.soft = {
			...
			content:
                ...
                <div id="pay_product_view">\
                    ...
                    <div class="libPay-layer-item">\
                        <div class="libPay-line-item proS" id="libPay-theme-tips"></div>\
                        ...
                        <div class="libPay-line-item" id="libPay-qcode-box" style="margin:20px 0 20px 0">\
                          ...
                          <div class="libPay-qcode-right">\
                              ...
                              <div class="pay-wx" style="height:132px;width: 132px;margin-right: 20px;" id="PayQcode" ></div>\
									...
									if(!$.isEmptyObject(temporary_coupon) && !$('.exclusive_coupon_popup').length) {
										if(!(use_coupon_id !== null && use_coupon_id.indexOf(temporary_coupon.id) > -1)) {
											...
											bt.open({
												...
												content: '<div class="exclusive_coupon_title">恭喜你获得专属优惠劵</div>\
												...
												success: function (layero, index) {
													...
													$(layero).before($('<img class="exclusive_coupon_light" src="/static/img/exclusive_coupon_light.svg" alt="" />').css({
														...
														'top': $(layero).css('top').match(/\d+/) - 170+'px',
													}))
													...
													$(layero).find('.exclusive_coupon_btn').click(function () {
														var _cycle = $('.pay-cycle-btns.active').data('data'),//周期数据
																...
																_price = 0
														_cycle.nums.forEach((item,index) => {
															if(item.count == _nums){
																_price = item.price
															}
														});
	...
	get_product_change: function (idx, btype) {
		...
		if (btype !== 'ver') {
			...
			if (btype !== 'plugin') {
				//企业版、运维版添加报告链接
				if (_data['type'] == 'ltd' || _data['type'] == 'ops') {
					href =
				...
				$.each(_data['rlist'], function (index, item) {
					_list += '<div class="pro-introduce"><span class="glyphicon glyphicon-ok"></span><span>' + item + '</span></div>';
				});
				_html =
	...
	privilege_contrast: function () {
		...
		that.get_soft_list(undefined,undefined,undefined,function (res) {
			var vips_list = res.remarks.vips_list
			var function_title_html = '',
					pro_cont_html = '',
					ltd_cont_html = '',
					ops_cont_html = '';
			$.each(vips_list, function (key, value) {
				function_title_html += '<tr><td>' + key + '</td></tr>';
				pro_cont_html += '<tr><td><i class="' + (!value.pro ? 'error-icon' : 'success-icon') + '"></i></td></tr>';
				ltd_cont_html += '<tr><td><i class="' + (!value.ltd ? 'error-icon' : 'success-icon') + '"></i></td></tr>';
				ops_cont_html += '<tr><td><i class="' + (!value.vltd ? 'error-icon' : 'success-icon') + '"></i></td></tr>';
			})
			bt.open({
				type: 1,
				title: false,
				...
				closeBtn: 2,
				shadeClose: false,
				content: '<div class="privilege_contrast_title">\
						<img src="/static/img/privilege_contrast_title.png" width="290px" alt="" />\
					</div>\
					<div class="privilege_contrast_cont">\
						<table class="function_table">\
							<thead>\
								<tr>\
									<th><div class="title_type">功能特权</div></th>\
								</tr>\
							</thead>\
							<tbody class="function_title"></tbody>\
						</table>\
						<table class="pro_table">\
							<thead>\
								<tr>\
									<th>\
										<div class="title_type"><img src="/static/img/pro-icon.svg" alt="" />专业版</div>\
									</th>\
								</tr>\
							</thead>\
							<tbody class="pro_cont"></tbody>\
						</table>\
						<table class="ltd_table">\
							<thead>\
								<tr>\
									<th>\
										<span class="recommend-icon"></span>\
										<div class="title_type"><img src="./static/img/ltd-icon.svg" alt="" />企业版</div>\
										<button type="button" class="ltd_btn">立即购买</button>\
									</th>\
								</tr>\
							</thead>\
							<tbody class="ltd_cont"></tbody>\
						</table>\
						<table class="ops_table">\
							<thead>\
								<tr>\
									<th>\
										<div class="title_type"><img src="./static/img/ltd-icon.svg" alt="" />企业运维托管</div>\
										<button type="button" class="ops_btn"><span>立即购买</span></button>\
									</th>\
								</tr>\
							</thead>\
							<tbody class="ops_cont"></tbody>\
						</table>\
					</div>',
					success: function (layero,index) {
						$(layero).find('.layui-layer-content').css({'padding': '25px 40px 0 40px','line-height': '22px','color': '#666'})
						var function_title = $('.function_title'),
								pro_cont = $('.pro_cont'),
								ltd_cont = $('.ltd_cont'),
								ops_cont = $('.ops_cont');
						function_title.html(function_title_html);
						pro_cont.html(pro_cont_html);
						ltd_cont.html(ltd_cont_html);
						ops_cont.html(ops_cont_html);
						$('.ltd_btn').click(function () {
              if($('.libPay-view').length){
                $('.libPay-menu-type.lib_ltd').click();
              }else{
                that.product_pay_view({ totalNum: 51,limit: 'ltd',pid: 100000032,is_coupon: false })
              }
							$('.libPay-menu-type.lib_ltd').click();
							layer.close(index);
						})
	...
	show_pay_code: function (idx, couponId) {
      ...
      discount_amount = org_price - cash_fee;
		...
		$('.libPayCycle')
			...
			.append('<i>' + _num + '台</i><span class="org_price">原价: ' + org_price + '元</span>'+ (_coupon ?'<span class="discount_amount">已使用优惠券，立减'+ parseFloat(_coupon.val2) +'元</span>' : ''));
		...
		switch (idx) {
			...
			case 2: //余额支付
				...
				$('.availableBalanceBtn').unbind('click').click(function(){
					...
					bt.confirm({msg:'您确定要使用余额支付吗？',title:'余额支付'},function(){
						var params = {
							num: _num,
							cycle: _cycle.cycle,
							uid: _data.data.uid,
						...
						bt.send('create_with_credit_by_panel', 'auth/create_with_credit_by_panel', params, function (res) {
							...
							if(res.success){
								that.product_pay_monitor({
									...
									idx: idx,
									pid: _data.data.pid,
									wxoid: _data.data.wxoid,
									name: _product.name,
									title: _product.title,
									num: _num,
									cycle: cash_fee < 1 ? '7天' : that.pro.conver_unit(_cycle.cycle),
									rlist: _product.rlist,
									username: $('.info_value').text()
								});
							}else{
	...
	get_rscode_other: function (pid, price, cycle, type) {
		...
		$.post(
			'/auth?action=create_plugin_other_order',
			{
				pid: pid,
				cycle: cycle,
				type: type,
			...
			if ($.inArray(name, arrs) >= 0 || name.indexOf('php-') >= 0) {
				...
				bt.open({
					...
					content:
						...
						lan.soft.install_version +
						"：<select id='SelectVersion' class='bt-input-text' style='margin-left:30px;width:200px;'>" +
						SelectVersion +
						'</select></div>\
				    </div>',
					yes: function (indexs, layers) {
	 ...
	 * @param that
	 */
	install_soft: function (item, that) {
		//安装单版本
			...
			function (rdata) {
				...
				layer.open({
                    ...
                    <div class="bt-form c7 install-three-plugin">\
                        <div class="install-header-title"><span class="install-icon"></span><span class="install-tips">' +
						title +
						'【<span title="' +
						data.title +
						'">' +
						data.title +
						'</span>】插件，是否继续？</span></div>\
                        <div class="plugin-title"><div class="item"><span>最新版本：</span><span style="color:#20A53a;">' +
						data.title +
						'v' +
						info.m_version +
						'.' +
						info.version +
						'</span></div><div class="item"><span>文件大小：</span><span>' +
						...
						data.ps +
						'</div>\
                           <div class="plugin-info-title">更新内容( V' +
						info.m_version +
						'.' +
						info.version +
						' )：</div>\
                           <pre class="plugin-info-content" style="margin-bottom: 0">' +
						(info.update_msg || '无') +
						'</pre>\
                    ...
                    <ul class="help-info-text c7">\
						...
						'】，网址 <a href="' +
						data.home +
						'" target="_blank" class="btlink" style="text-decoration: revert;">' +
						data.home +
						'</a></li>\
                        <li>如果已存在此插件，文件将被替换！</li>\
                        <li style="color:red" class="' +
						(JSON.stringify(data.dependnet) === '{}' ? 'hide' : '') +
						'">请手动按照插件依赖环境，如果未安装，将无法正常使用该插件</li>\
                        <li style="color:red" class="' +
						(data.update && data.name === 'mysql' ? '' : 'hide') +
						'">更新数据库有风险,建议在更新前,先备份您的数据库！</li>\
                        <li style="color:red" class="' +
						(!data.update ? 'hide' : '') +
						'">更新过程可能会导致服务中断,请须知</li>\
                        <li style="color:red" class="' +
						(!data.update ? 'hide' : '') +
						'">建议您在服务器负载闲时进行软件更新</li></li>\
                        <li class="' +
						(data.update ? 'hide' : '') +
						'">安装过程可能需要几分钟时间，请耐心等候!</li>\
                    </ul>\
                </div>',
					success: function (layers, indexs) {
					...
					content:
						...
						(JSON.stringify(beta) === '{}' || is_beta
							...
							: '<div class="cut_plugin_version "><div class="plugin_title_info ' +
							  ...
							  (info.beta === 2 ? '无版本信息' : info.update_msg) +
							  '</pre></div>' +
						...
						if (obj.val() == 'ip') {
							obj
								...
								.append(
									'<textarea id="' +
										subid +
										'" class="bt-input-text mr5" type="text" name="address" placeholder="多个IP使用逗号(,)分隔" style="line-height: 20px;width: 200px;height: 70px; display: block;margin-top: 10px;"></textarea>'
								);
						}
					},
				},
	...
	get_quota_config: function (type) {
		return {
			...
			template: function (row, index) {
				...
				return (
					'<div class=""><div class="progress mb0 cursor" style="height:12px;line-height:12px;vertical-align:middle;border-radius:2px;margin-top:3px;" title="当前已用容量：' +
