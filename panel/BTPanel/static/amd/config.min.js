...
define(["require", "exports", "./snabbdom", "./public/public"], function (require, exports, snabbdom_1, public_1) {
	...
	var Config = (function (_super) {
		...
		function Config() {
			...
			_this.panelSiteList = [];
			_this.taskTypeList = [
				{ title: '网站证书到期', value: 'ssl', model: 'site_push' },
			...
			_this.apiInfo = {
				...
				getMessageChannel: ['config/get_msg_configs', '获取消息通道状态'],
								...
								content: {
									...
									template: function () {
										return ((0, snabbdom_1.jsx)("div", { class: this.$class('bt-form pd25') },
											...
											this.$line({ title: '面板SSL', width: '65px' }, this.$switch({
												model: 'panel_ssl',
		...
		Config.prototype.setAlarmView = function () {
			return __awaiter(this, void 0, void 0, function () {
				var config, channelInfo, email, feishu, weixin, dingding, sms, isChannel, error_15;
				var _this = this;
				return __generator(this, function (_a) {
					switch (_a.label) {
						case 0:
							...
							email = channelInfo.mail.data, feishu = channelInfo.feishu.data, weixin = channelInfo.weixin.data, dingding = channelInfo.dingding.data, sms = channelInfo.sms.data;
							isChannel = !(email && email.receive) && !(feishu && feishu.feishu_url) && !(weixin && weixin.weixin_url) && !(dingding && dingding.dingding_url) && $.isEmptyObject(sms);
							if (isChannel)
								return [2, this.$msg({ msg: '请在消息通道中至少设置一个通道', icon: 2 })];
							_a.label = 1;
						case 1:
							_a.trys.push([1, 3, , 4]);
							return [4, this.$open({
		...
		Config.prototype.renderAlertModelConfigInfo = function (mode) {
			...
			if (mode.setup) {
				if (mode.name != 'sms' && mode.name != 'wx_account') {
					...
					if (res.is_subscribe && res.is_bound)
						boundCheck = "\u5FAE\u4FE1\u8D26\u53F7\u3010".concat(res.nickname, "\u3011,\u4ECA\u65E5\u5269\u4F59\u53D1\u9001\u6B21\u6570:").concat(res.remaining);
					_info = boundCheck;
				}
			}
			else {
						...
						template: function (row) {
							switch (row.type) {
								...
								case 'panel_pwd_endtime':
									return "<span>\u5269\u4F59\u65F6\u95F4\u5C0F\u4E8E".concat(row.cycle, "\u5929,\u6BCF\u5929\u68C0\u6D4B\u4E00\u6B21</span>");
							...
							bt.confirm({ title: '批量删除任务', msg: '批量删除后将无法恢复，是否继续？', icon: 0 }, function (index) {
								...
								that.start_batch({}, function (list) {
									...
									for (var i = 0; i < list.length; i++) {
										var item = list[i];
										html += '<tr><td>' + (item.project == '' ? item.title : item.project + item.title) + '</td><td><div style="float:right;"><span style="color:' + (item.request.status ? '#20a53a' : 'red') + '">' + item.request.msg + '</span></div></td></tr>';
									}
		...
		Config.prototype.switchPushType = function (config, formData) {
			...
			this.alertListModule.forEach(function (mod, i) {
				...
				_checklist.push({
					...
					event: function (formData, element, thatE) {
						thatE.config.form[5].group[i].value = !formData[mod.name] ? 0 : 1;
					}
				});
			});
			if (!formData.id) {
			...
			config[6].hide = formData.type === 'panel_safe_push' ? false : true;
			config[0].group.value = formData.type;
			config[1].group.list = siteList;
			config[5].group = _checklist;
			return config;
		};
		Config.prototype.setAlertTaskConfig = function (row) {
			return __awaiter(this, void 0, void 0, function () {
				...
				var _this = this;
				return __generator(this, function (_a) {
					_config = this.switchPushType(JSON.parse(JSON.stringify(this.alertConfigForm)), row);
		...
		Config.prototype.eventBind = function () {
			return __awaiter(this, void 0, void 0, function () {
				...
				return __generator(this, function (_a) {
					...
					$('.login-alarm-line').on('click', '.setAlarmMail, .setAlarmMailBtn', function () { return _this.setAlarmView(); });
