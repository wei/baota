...
define("public/public", ["require", "exports", "snabbdom", "public/utils"], function (require, exports, snabbdom_2, utils_1) {
	...
	var Public = (function (_super) {
		...
		Public.prototype.renderChannelLine = function (config) {
			var WXAccount = $('.setMessageChannelWXAccount');
			var mailMessage = $('.setMessageChannelMail');
			var nailNailMessage = $('.setMessageChannelNailNail');
			var smsMessage = $('.setMessageChannelSMS');
			var message_channel_info = config.message_channel_info;
			var wx_account = message_channel_info.wx_account.data, email = message_channel_info.mail.data, dingding = message_channel_info.dingding.data, weixin = message_channel_info.weixin.data, feishu = message_channel_info.feishu.data, sms = message_channel_info.sms.data;

			if (!(wx_account && wx_account['res'] && wx_account['res']['is_bound'] && wx_account['res']['is_subscribe'])) {
				WXAccount
					.text('微信公众号未配置')
					.removeClass('btlink')
					.addClass('bt_warning');
				...
				WXAccount
					.text('微信公众号已配置')
					.removeClass('bt_warning')
					.addClass('btlink');
			}
			if (!(email && email.receive)) {
				mailMessage
					.text('邮箱未配置')
					.removeClass('btlink')
					.addClass('bt_warning');
			...
			if (!(feishu && feishu.feishu_url) &&
				!(weixin && weixin.weixin_url) &&
				!(dingding && dingding.dingding_url)) {
				nailNailMessage
					.text('钉钉/飞书/企业微信未配置')
					.removeClass('btlink')
					.addClass('bt_warning');
		...
		Public.prototype.renderLoginAlarmLine = function (config) {
			...
			var email = message_channel_info.mail.data, feishu = message_channel_info.feishu.data, weixin = message_channel_info.weixin.data, dingding = message_channel_info.dingding.data, sms = message_channel_info.sms.data,wx_account = message_channel_info.wx_account;
			var isChannel = !$.isEmptyObject(email) ||
				!$.isEmptyObject(feishu) ||
				!$.isEmptyObject(weixin) ||
				!$.isEmptyObject(dingding)||
				!$.isEmptyObject(sms)||
				!(wx_account && wx_account['res'] && wx_account['res']['is_bound'] && wx_account['res']['is_subscribe']);

			var data = message_channel_info[login_alarm_info];
			var tits = [];
			$.each(message_channel_info, function (key, item) {
				tits.push(item.title);
			});
			$('.login-alarm-line .line-row-tips span').text(tits.join('/'));
			if (isChannel && data) {
				$('.login-alarm-line .setAlarmMail')
					.removeClass('bt_warning')
					.addClass('btlink');
				$('.login-alarm-line .setAlarmMail').text("".concat(data.title, "\u5DF2\u914D\u7F6E"));
			}
			else {
				$('.login-alarm-line .setAlarmMail')
					.addClass('bt_warning')
					.removeClass('btlink');
				$('.login-alarm-line .setAlarmMail').text("\u544A\u8B66\u901A\u77E5\u672A\u914D\u7F6E");
			}
		};
		return Public;
	}(utils_1.default));
