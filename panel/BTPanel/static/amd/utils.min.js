...
define("public/public", ["require", "exports", "snabbdom", "public/utils"], function (require, exports, snabbdom_2, utils_1) {
    ...
    var Public = (function (_super) {
        ...
        Public.prototype.renderChannelLine = function (config) {
            ...
            var email = message_channel_info.mail.data, dingding = message_channel_info.dingding.data, weixin = message_channel_info.weixin.data, feishu = message_channel_info.feishu.data, sms = message_channel_info.sms.data;
            if ($.isEmptyObject(email)) {
                mailMessage.text('邮箱未配置').removeClass('btlink').addClass('bt_warning');
            }
            else {
                mailMessage.text('邮箱已配置').removeClass('bt_warning').addClass('btlink');
            }
            if ($.isEmptyObject(dingding) && $.isEmptyObject(weixin) && $.isEmptyObject(feishu)) {
                nailNailMessage.text('钉钉/飞书/企业微信未配置').removeClass('btlink').addClass('bt_warning');
            }
            else {
                nailNailMessage.text('钉钉/飞书/企业微信已配置').removeClass('bt_warning').addClass('btlink');
            }
            if ($.isEmptyObject(sms)) {
                smsMessage.text('短信未配置').removeClass('btlink').addClass('bt_warning');
            }
            else {
                smsMessage.text('短信已配置').removeClass('bt_warning').addClass('btlink');
            }
        };
        Public.prototype.renderLoginAlarmLine = function (config) {
            ...
            var email = message_channel_info.mail.data, dingding = message_channel_info.dingding.data, weixin = message_channel_info.weixin.data;
            var isChannel = email || dingding || weixin;
            if (isChannel && isSetEmail) {
                setAlarmMail.show().text('邮箱已配置').removeClass('bt_warning').addClass('btlink');
                interval.hide();
                setAlarmNailNail.hide();
            }
            else if (isChannel && isSetdingding) {
                setAlarmNailNail.show().text('钉钉/企业微信已配置').removeClass('bt_warning').addClass('btlink');
                interval.hide();
                setAlarmMail.hide();
            }
            else {
                ...
                setAlarmMail.text('邮箱未配置').removeClass('btlink').addClass('bt_warning');
                setAlarmNailNail.text('钉钉/企业微信未配置').removeClass('btlink').addClass('bt_warning');
            }
        };
        return Public;
    }(utils_1.default));
